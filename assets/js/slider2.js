function Sim1(SimID){
	id = document.getElementById(SimID);
	if (id) {
		this.SldrRoot = id;
	}
	else{
		this.SldrRoot = document.querySelector('.slider1')
	};

	//obsects

	this.sliderList = this.SldrRoot.querySelector('.sim-slider-list1');
	this.SldrElements = this.SldrRoot.querySelectorAll('.slim-slider-element1');
	this.SLdrElementFirst = this.SldrRoot.querySelector('.slim-slider-element1');
	this.SldrArrowLeft = this.SldrRoot.querySelector('div.arrow-left1');
	this.SldrArrowRight = this.SldrRoot.querySelector('div.arrow-right1');
	this.SldrDots = this.SldrRoot.querySelector('div.dots1');
	console.log('объекты');
...
Sim1.defaults = {
	...
	arrows: false, //slide using arrows
	dots: false //show/hide dots
}

Sim1.prototype.PrevElem = function(differenceBetweenPoints){
...
Sim1.prototype.NextElement = function(differenceBetweenPoints){
	differenceBetweenPoints = differenceBetweenPoints || 1;
	let PreviousElement = this.currentElement;
	this.currentElement += differenceBetweenPoints;
...
Sim1.initialize = function(that){
	...
	if(!that.options.loop){
		...
		//that.sliderList.addEventListener('mouseover', function(){clearInterval(that.autoScroll), false});
